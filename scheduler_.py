...
class PumpScheduler:
    ...
    def _watering_job(self, duration_seconds, schedule_id):
        ...
        try:
            # Start filling (pump on) - sends webhook via pump_controller
            self.pump.set_low()
            ...
            # Stop filling (pump off) - sends webhook via pump_controller
            self.pump.set_high()
    ...
    def _check_and_start_schedule(self, schedule_id):
        ...
        # Check if we should run (time matches)
        if current_hour == schedule_hour and current_minute == schedule_minute:
            ...
            # OPTIONAL: Check if the schedule time has already passed today
            # If so, start it anyway (useful for testing)
            if current_hour > schedule_hour or (current_hour == schedule_hour and current_minute > schedule_minute):
                logger.info(f"Schedule {schedule_id} time has already passed today, starting anyway for testing!")
                duration = schedule['duration']
                import threading
                def start_schedule_immediately():
                    logger.info(f"Starting immediate watering for schedule {schedule_id} (late start) ({duration}s)")
                    self._watering_job(duration, schedule_id)
                thread = threading.Thread(target=start_schedule_immediately, daemon=True)
                thread.start()
                return True
    ...
    def toggle_schedule(self, schedule_id, enabled):
        ...
        # Update the enabled state in memory FIRST
        self.schedules[schedule_id]['enabled'] = enabled
        
        # If enabling, check if schedule should run NOW (before resuming the job)
        if enabled:
