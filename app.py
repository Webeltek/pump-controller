...
def pump_on():
    """Turn pump on (set empty/low level)"""
    try:
        pump.set_empty()  # Empty level = pump runs
        logger.info("Pump turned ON via API - Level set to EMPTY")
        ...
        return jsonify({
            'success': True, 
            'status': 'on', 
            'message': 'Pump turned on - Level set to EMPTY',
...
def set_empty():
    try:
        pump.set_empty()
        return jsonify({'success': True, 'level': 'EMPTY', 'pump': 'RUNNING'})
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500
